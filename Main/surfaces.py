...
class Tile(pygame.sprite.Sprite):
    def __init__(self, position, color=(100, 100, 100), solid=True):
        super().__init__()
...
class Surface(pygame.sprite.Sprite):
    def __init__(self):
        ...
        self.tile_grid= [[None for _ in range(self.level_layout[0].__len__())] for _ in range(self.level_layout.__len__())]
        self.dark_overlay = pygame.Surface(screen.get_size(), pygame.SRCALPHA)
        print(self.tile_grid[0].__len__())
        ...
        for row_index, row in enumerate(self.level_layout):
            for col_index, tile_type in enumerate(row):
                if tile_type == '1':
                    ...
                    tile = Tile(position)
                    self.tile_grid[row_index][col_index] = Tile(position)
    ...
    def draw(self, players):
        ...
        for player in players:
            ...
            self.cast_light(player, radius)

        screen.blit(self.dark_overlay, [0, 0])

    def cast_light(self, source, radius):
        MAX_ALFA = 280
        for angle in range(0, 360, 5):                 #Calculating the amount of rays
            ...
            for i in range(0, radius, 12):
                ...
                distance_from_source = math.sqrt((x - (source.position[0] + 25)) ** 2 + (y - (source.position[1] + 25)) ** 2)
                alpha = max(50, min(MAX_ALFA, int(MAX_ALFA * (1.3 - distance_from_source / radius))))

                # Draw the light point with gradient
                pygame.draw.circle(self.dark_overlay, (0, 0, 0,MAX_ALFA-alpha), point, 15)
